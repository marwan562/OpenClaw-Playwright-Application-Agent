...
export class WuzzufPlatform implements JobPlatform {
  ...
  public async openJob(url: string): Promise<void> {
    logger.info(`Navigating to Wuzzuf Job URL: ${url}`, 'WuzzufPlatform');
    await this.browserCtx.page.goto(url, { waitUntil: 'domcontentloaded' });
    await this.browserCtx.waitUntilStable();
    await this.browserCtx.takeScreenshot('wuzzuf_job_page_opened');
    this.jobLocation = await this.extractJobLocation();
  }

  public async detectApplyMethod(): Promise<boolean> {
    logger.info('Detecting Wuzzuf application method...', 'WuzzufPlatform');
    for (const selector of SELECTORS.applyButton) {
      if (await this.browserCtx.isVisible(selector)) {
        logger.info(`Apply button detected using selector: "${selector}"`, 'WuzzufPlatform');
        return true;
      }
    }
    logger.warn('Wuzzuf Apply button NOT found on this page.', 'WuzzufPlatform');
    return false;
  }

  public async openApplyForm(): Promise<boolean> {
    logger.info('Attempting to open Wuzzuf application form/modal...', 'WuzzufPlatform');
    let applySelector = '';
    for (const selector of SELECTORS.applyButton) {
      if (await this.browserCtx.isVisible(selector)) {
    ...
    if (!applySelector) {
      logger.error('Cannot open Wuzzuf form: No apply button found.', null, 'WuzzufPlatform');
      return false;
    }
    ...
    // Check if Wuzzuf redirects or opens a new layout
    await this.browserCtx.takeScreenshot('wuzzuf_form_opened');
    return true;
  }

  public async fillApplication(): Promise<void> {
    logger.info('Starting Wuzzuf application filling...', 'WuzzufPlatform');
    ...
    while (true) {
      logger.info(`---- Processing Wuzzuf Step ${step} ----`, 'WuzzufPlatform');
      await this.browserCtx.waitUntilStable();
      await this.browserCtx.takeScreenshot(`wuzzuf_step_${step}_start`);

      // Check review/submit page
      const isReviewPage = await this.detectReviewPage();
      if (isReviewPage) {
        logger.info('Detected final Wuzzuf Review / Submit page. Stopping automation.', 'WuzzufPlatform');
        await this.browserCtx.takeScreenshot('wuzzuf_final_review_page');
        break;
      }
      ...
      if (fields.length === 0) {
        logger.warn('No form fields detected. Checking for next button.', 'WuzzufPlatform');
      } else {
        for (const field of fields) {
          try {
            await this.processField(field);
          } catch (err) {
            logger.error(`Failed to process field "${field.label}"`, err, 'WuzzufPlatform');
          }
        }
      }
      ...
      if (currentHtml === previousStepHtml) {
        sameStepCount++;
        if (sameStepCount >= 3) {
          logger.error('Wuzzuf automation stuck on the same page. Field validation error.', null, 'WuzzufPlatform');
          await this.browserCtx.takeScreenshot('wuzzuf_stuck_validation');
      ...
      if (!proceeded) {
        logger.warn('Could not find Next or Review button.', 'WuzzufPlatform');
        break;
      }

      step++;
      if (step > 15) {
        logger.error('Safety step limit exceeded.', null, 'WuzzufPlatform');
        break;
      }
    }
  }
  ...
  public async review(): Promise<void> {
    logger.info('Wuzzuf Application filled and stopping at Review.', 'WuzzufPlatform');
  }

  public async close(): Promise<void> {
    logger.info('Wuzzuf platform worker complete.', 'WuzzufPlatform');
  }

  private async processField(field: FormField): Promise<void> {
    logger.info(`Wuzzuf field: Type: "${field.type}" | Label: "${field.label}"`, 'WuzzufPlatform');
    ...
    switch (field.type) {
      ...
      case 'select':
        if (await field.element.evaluate((node: HTMLElement) => node.tagName.toLowerCase() === 'select')) {
          ...
          // Custom selector click fallback
          await field.element.click();
          await page.waitForTimeout(500);
          const optionText = value;
          const optionLocator = page.locator(`[role="option"]:has-text("${optionText}"), li:has-text("${optionText}")`).first();
